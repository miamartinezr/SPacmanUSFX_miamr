	...
	while (getline(file, line)) {
		...
		for (unsigned int x = 0; x < chars.size(); x++) {
			...
			case 'p':
				...
				break;
			case 'b':
				newObject = new Fantasma(fantasma1Texture, x * 25, y * 25, 25, 25, anchoPantalla, altoPantalla, 5);
				newObject->setParametrosAnimacion(4);
				break;
			case 'a':
			...
			case 'c':
				...
				break;
			case 'd':
				newObject = new Fantasma(fantasma4Texture, x * 25, y * 25, 25, 25, anchoPantalla, altoPantalla, 5);
				newObject->setParametrosAnimacion(4);
				break;
			case 'f':
				//newObject = new Fruta();
...
void MapGenerator::populate(std::vector<GameObject*>& _vectorObjetosJuegoGM)
{
	...
	for (auto iloj = listaObjetosJuego.begin(); iloj != listaObjetosJuego.end(); ++iloj) {
		_vectorObjetosJuegoGM.push_back(*iloj);
